        ...
        if topic == "/tf":
            ...
            for transform in msg.transforms:
                ...
                if is_initial_pose:
                    ...
                    dist_frames = is_close_pose(prev_trans, trans, interval[0], interval[1])
                    # print("too close or too far")
                    far_times = math.ceil(dist_frames/interval[1])
                    # print(dist_frames,far_times)
                    # print(np.r_[prev_trans.x, prev_trans.y, prev_trans.z, rot.x, rot.y, rot.z, rot.w])
                    for index in range(1,far_times):
                        insert_frame = np.r_[prev_trans.x + index/far_times*(trans.x-prev_trans.x), \
                                             prev_trans.y + index/far_times*(trans.y-prev_trans.y), \
                                             prev_trans.z + index/far_times*(trans.z-prev_trans.z), \
                                             rot.x,  \
                                             rot.y, \
                                             rot.z, \
                                             rot.w]                                            
                        pose_list.append(insert_frame) 
                        # print(insert_frame)               
                    # print(np.r_[trans.x, trans.y, trans.z, rot.x, rot.y, rot.z, rot.w])
                pose_list.append(np.r_[trans.x, trans.y, trans.z, rot.x, rot.y, rot.z, rot.w])
                # print(np.r_[trans.x, trans.y, trans.z, rot.x, rot.y, rot.z, rot.w])
                prev_trans = trans

    # print(np.array(pose_list))          
